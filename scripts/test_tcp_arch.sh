...
# Script de prueba específico para Arch Linux
# Uso: ./test_tcp_arch.sh
...
echo "=== PRUEBA TCP SERVER/CLIENT PARA ARCH LINUX ==="
...
# Iniciar servidor en segundo plano con mejor manejo
nohup "$BIN_DIR/tcp_server" "$TCP_PORT" "$OUTPUT_FILE" 4096 > server.log 2>&1 &
SERVER_PID=$!
...
# Limpieza
rm -f "$OUTPUT_FILE" server.log client.log nohup.out 2>/dev/null || true
